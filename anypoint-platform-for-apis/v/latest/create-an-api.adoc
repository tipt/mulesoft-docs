...
In this section, you start by creating a RESTful API for a web service that handles t-shirt orders and order tracking with an hypothetical database. +
The point of the whole tutorial is to show how to integrate your Mule application to a SOAP service by consuming a SOAP API.

In this part, you focus on designing a REST API by writing a RAML definition to match your SOAP service's WSDL.
...
Go to link:/anypoint-platform-for-apis/create-a-proxy-for-an-api[Create a Proxy].
