...
[NOTE]
Check the link:/mule-user-guide/v/3.8-m1/dataweave-types#type-coercion-table[type coercion table] to see what conversions between what types are allowed in DataWeave.
...
The read operator can process data that is in the input but not being parsed, for example if it's in a CDATA object or being treated as a literal string. The first argument takes the string that must be read, the second is the format in which it's written.
...
The write operator can compose data into any given format, irregardless of what the transformation is doing at a larger scale. The result is then treated as a string literal within the rest of the output. You could for example include a CSV field within one element of an XML output.
