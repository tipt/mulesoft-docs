...
|*mule-deploy.properties* | This file may contain two optional configuration properties equivalent to the ones in a Mule application: *redeployment.enabled* and *loader.overrides*. See link:/mule-user-guide/v/3.8-m1/mule-application-deployment-descriptor#deployment-descriptor-in-a-domain[Deployment Descriptor in a Domain] for more details.
|===
