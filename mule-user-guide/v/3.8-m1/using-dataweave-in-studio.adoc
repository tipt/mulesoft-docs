...
* *you can manually specify it directly in the input section of your Transform Message component*

=== Defining Metadata Via the UI

If the metadata definition is missing, a notification on the input section of the Graphical UI will advise you to provide it, click the *Define Metadata* link. Otherwise, at any time you can also right-click the input section and select *Set Metadata* to access the same options.
...
The main field you must configure is the *type* of your input. If the type of your input is undeclared or not understood it will default to 'application/java', a warning will be logged.
...
Some input formats such as CSV also allow you to then set some additional parameters in the <<reader configuration>>.


If your metadata is missing information about any variables, inbound properties, outbound properties, session variables or record variables that you know will exist in the incoming mule message, you can manually add these too in the editor. Just right-click on one of these categories in the input section of the properties editor and select *Add Flow Variable*, *Add Inbound Property* or whatever the element you want to add may be.
...
* Right clicking on the input section and selecting *Reader Configuration* to access a menu
+
...
The properties you can configure depend on the type of your input.
...
When defining an input of type CSV, there are a few optional parameters you can use to customize how the data will be parsed.
...
* `nullValueOn`: boolean that defines if a tag with empty or blank text should be read as null. 'none' by default.
...
Refer to the list above for a full reference of all the available parameters.
...
Just as with <<Defining Input Structures>>, if other elements in your Mule flow expose metadata about their input and output, then this information should already be available to the *Transform Message* component and the UI section should display it. If they don't, there are three things you can do:

* *you can configure the elements themselves* so that they expose this information
* *you can manually specify it* in the output section of your Transform Message component
* *you can skip specifying the definition* and directly set an output type in the output directive of your dataweave code

If the metadata definition is missing, a notification on the output section of the Graphical UI will advise you to provide it, click the *Define Metadata* link. Otherwise, at any time you can also right-click the output section and select *Set Metadata* to access the same options.
...
There are a number of fields you can set up, depending on the selected format.

Some input formats such as CSV require that you then set some additional parameters in the <<writer configuration>>.
...
See link:/mule-user-guide/v/3.8-m1/dataweave-language-introduction#output-directive[output directive] for details on this.
