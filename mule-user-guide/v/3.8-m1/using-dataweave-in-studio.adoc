...
If the Mule flow doesn't expose Metadata about the elements you need from the incoming message, you can also manually specify it directly in the input section of your Transform Message component.

If a metadata definition is missing, a notification will advise you to provide one, click the *Define Metadata* link. Otherwise, at any time you can also right-click your input and select *Set Metadata* to access the same options.
...
If the type of your input is undeclared or not understood it will default to 'application/java', a warning will be logged.
...
* Adding properties in the XML source of your Mule project

image:dw_reader_configuration_select.png[reader conf]


In XML, you can add properties by adding 'dw:reader-property' child elements within:
...
The available properties will depend on the type of your input.
