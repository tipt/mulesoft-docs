...
For defining the Output structure, you also have a third option if you plan to create your transform entirely via <<the DataWeave text editor>>:

* *you can skip specifying the definition* and directly set an output type in the link:/mule-user-guide/v/3.8-m1/dataweave-language-introduction#output-directive[output directive] of your DataWeave code
...
[TIP]
See reference docs for the schema structures required for link:/mule-user-guide/v/3.8-m1/dataweave-flat-file-schemas[Flat-File] and link:/mule-user-guide/v/3.8-m1/dataweave-edi-schemas[EDI].
...
* Copybook type definitions actually use flat-file schemas, Migration from copybook to fixedwidth is supported with the exception of some options, link:/mule-user-guide/v/3.8-m1/dataweave-flat-file-schemas[see which ones].
...
The properties you can configure depend on the type of your input, see the sections below.
...
When defining an input of type CSV, there are a few optional reader parameters you can use to customize how the data will be parsed.
...
When defining an input of type Flat File, there are a few optional reader parameters you can add to the input directive to customize how the data will be parsed.
...
In the XML editor, if you want to parse flat-file inputs with custom modifiers, you must set these up as child elements of the DataWeave component, like in the example below:
...
[TIP]
See link:/mule-user-guide/v/3.8-m1/dataweave-flat-file-schemas[DataWeave Flat File Schemas] for instructions and examples on how to create the required schema.
...
You can also access this tab by right clicking on the root of the input section and selecting *Edit Sample Data*.
