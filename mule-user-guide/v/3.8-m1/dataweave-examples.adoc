...
Below are a series of advanced examples using link:/mule-user-guide/v/3.8-m1/dataweave-language-introduction[DataWeave Language]. For a more basic and closely guided step by step series of exercises that take you through how to use this feature via the Anypoint Studio UI, see link:/mule-user-guide/v/3.8-m1/dataweave-tutorials[DataWeave tutorials].

== Using the Map Operatorisi

This example takes an XML list of books and outputs a JSON version of the same. It goes through each 'book' element in the input using the link:/mule-user-guide/v/3.8-m1/dataweave-operators#map[*map* operator].  Values that are numeric are defined as numbers using the link:/mule-user-guide/v/3.8-m1/dataweave-operators#as[As operator] to avoid having them treated as strings. As JSON can't have duplicate keys, for elements that may be duplicate in the inbound XML, only the first is selected to be put into the output.
...
In this example, the input contains sensitive information that should be removed. The transform replicates the inbound structure but uses a simple link:/mule-user-guide/v/3.8-m1/dataweave-operators#remove[*remove* operation] to take away specific key:value pairs.

The example goes through the whole set of elements in the input using a link:/mule-user-guide/v/3.8-m1/dataweave-operators#map[*map* operation].
...
In this example, the input contains the same sensitive information as in the previous one, but instead of entirely removing the key:value pairs that contain it, the values are replaced with the string **. The transform replicates the inbound structure but uses a simple link:/mule-user-guide/v/3.8-m1/dataweave-operators#when-or-otherwise[*when* operation] to replace values when specific keys occur.
...
This example transforms a JSON array of objects, each representing a book, into XML. The link:/mule-user-guide/v/3.8-m1/dataweave-operators#map[*map* operator] carries out the same steps for each element in the input array. Through the use of @, attributes are added into the XML tags.

[TIP]
See link:/mule-user-guide/v/3.8-m1/dataweave-language-introduction#attribute-selector-expressions[Attribute selector expressions] for how to access XML attributes in DataWeave.
...
The example goes through the whole set of elements in the input using  a link:/mule-user-guide/v/3.8-m1/dataweave-operators#map[*map* operator]. It also uses the link:/mule-user-guide/v/3.8-m1/dataweave-operators#when-or-otherwise[*when* operator] to only select instances that include the field 'insurance'.
...
The example goes through the whole set of elements in the input using  a link:/mule-user-guide/v/3.8-m1/dataweave-operators#map-object[*map object* operator]. Note that it differs from the map operation in that map object processes both key and value of its input, rather than just the keys. It also uses the link:/mule-user-guide/v/3.8-m1/dataweave-operators#when-or-otherwise[*when* operator] together with an link:/mule-user-guide/v/3.8-m1/dataweave-operators#and[*and*] to pick out the specific keys that need to change, and the link:/mule-user-guide/v/3.8-m1/dataweave-operators#as[*as* operator] to coerce its type to string.
...
This example converts an XML input to a JSON output that is structured differently and that contains URL links that are built from concatenating input content defining a few link:/mule-user-guide/v/3.8-m1/dataweave-language-introduction#define-constat-directive[constat directives] in link:/mule-user-guide/v/3.8-m1/dataweave-language-introduction#the-dataweave-header[The DataWeave Header]. The transform also creates a few fields that are conditional and are only present in the output when they exist in the input.
...
This example takes an XML input and parses it into a different XML arrangement. After a single 'header' element is copied, a link:/mule-user-guide/v/3.8-m1/dataweave-operators#map[*map* operation] carries out the same steps for each 'item': several fields are passed on without any changes, then the discount and subtotal fields are calculated with references to constants defined in the link:/mule-user-guide/v/3.8-m1/dataweave-language-introduction#the-dataweave-header[header directives] of the transform. A single set of subtotal, tax and total elements are created by performing a link:/mule-user-guide/v/3.8-m1/dataweave-operators#reduce[*reduce*] operation over all of the items in the "items" array, performing calculations that sometimes involve constants defined in the header. The link:/mule-user-guide/v/3.8-m1/dataweave-operators#as[*as* operation] is also used to coerce discound to a number and then performs link:/mule-user-guide/v/3.8-m1/dataweave-operators#basic-math-operations[basic math operations] on these numbers.
...
This example takes in an XML file that is grouped separating two types of elements: teachers and students, the transform uses the link:/mule-user-guide/v/3.8-m1/dataweave-operators#group-by[*Group By* operation] to make it into an XML where the top level grouping is "class". Within each class, the students and teachers that are registered in it are listed.

The example goes through the whole set of elements in the input using both a link:/mule-user-guide/v/3.8-m1/dataweave-operators#map-object[*map object* operation] and a link:/mule-user-guide/v/3.8-m1/dataweave-operators#map[*map* operation]. Note that they differ in that map object processes both key and value of its input, rather than just the keys. It also uses the link:/mule-user-guide/v/3.8-m1/dataweave-operators#filter[*filter* operation] to only show the attendees that have the class subject listed as a hobby. As each student may have multiple hobbies, the link:/mule-user-guide/v/3.8-m1/dataweave-language-basics#multi-value-selector[*Multi value* selector] is used to refer to them all.
...
In this example, there are three different input JSON files, these three all arrive in one single Mule message, occupying the payload and two link:/mule-user-guide/v/3.8-m1/dataweave-language-introduction#flow-variables-from-a-mule-message[flow variables]. The payload contains an array of book objects, one flow variable has a set of currency exchange rates, and the other one a query. The transform filters the first input using the conditions passed in the third input, then performs a link:/mule-user-guide/v/3.8-m1/dataweave-operators#map[*map* operation] to deal with each remaining object separately. Within this map, it defines two variables: *it* and *props*. Through the use of @, attributes are added into the XML tags. A second map operation inside the first one calculates the price of each book for each of the currencies provided in the second input. Another map operation displays each element in the "author" array as a separate <author></author> tag.


[TIP]
See link:/mule-user-guide/v/3.8-m1/dataweave-language-introduction#flow-variables-from-a-mule-message[flow variables] for how to access flow variables from the input and link:/mule-user-guide/v/3.8-m1/dataweave-language-introduction#attribute-selector-expressions[Attribute selector expressions] for how to access XML attributes in DataWeave.
...
The example defines a link:/mule-user-guide/v/3.8-m1/dataweave-functions-and-lambdas[function] in the link:/mule-user-guide/v/3.8-m1/dataweave-language-introduction#dataweave-header[dataweave header] and then calls it on the body. It goes through the whole set of elements in the input using  a link:/mule-user-guide/v/3.8-m1/dataweave-operators#map-object[*map object* operator]. Note that it differs from the map operation in that map object processes both key and value of its input, rather than just the keys. It also uses the link:/mule-user-guide/v/3.8-m1/dataweave-operators#when-or-otherwise[*when* and *otherwise* operators] to pick out only instances where an XML attribute exists, and to do nothing otherwise.
It also uses the link:/mule-user-guide/v/3.8-m1/dataweave-operators#match[*match* operator].
...
The following examples makes all the keys lower case. The link:/mule-user-guide/v/3.8-m1/dataweave-language-introduction#the-dataweave-header[DataWeave header] defines a link:/mule-user-guide/v/3.8-m1/dataweave-functions[function] that receives another function as an argument and then calls it in the body. It then only applies this function to the keys and not the values it processes. The function that is sent as an argument is a very simple one that uses the link:/mule-user-guide/v/3.8-m1/dataweave-operations#lower[lower operator] to set text to lower case.

The example goes through the whole set of elements in the input using a link:/mule-user-guide/v/3.8-m1/dataweave-operators#map-object[*map object* operator]. It also uses the link:/mule-user-guide/v/3.8-m1/dataweave-operators#when-or-otherwise[*when* and *otherwise* operators] to pick out only instances where an XML attribute exists, and to do nothing otherwise.
It also uses the link:/mule-user-guide/v/3.8-m1/dataweave-operators#match[*match* operator].
...
The example goes through the whole set of elements in the input using a link:/mule-user-guide/v/3.8-m1/dataweave-operators#map[*map* operator]. It also uses the link:/mule-user-guide/v/3.8-m1/dataweave-operators#default[*default* operator].


[NOTE]
Note that for this example to work, on the payload input you must set the reader configuration so that the 'nullValueOn' attribute is "empty". See link:/mule-user-guide/v/3.8-m1/using-dataweave-in-studio#reader-configuration[reader configuration] for instructions on how to set this.
