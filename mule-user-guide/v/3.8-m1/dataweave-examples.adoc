...
This example converts an XML input to a JSON output that is structured differently and that contains URL links that are built from concatenating input content with a few constants that are defined in link:/mule-user-guide/v/3.8-m1/dataweave-language-introduction[The DataWeave Header]. The transform also creates a few fields that are conditional and are only present in the output when they exist in the input.
...
This example takes an XML list of books and outputs a JSON version of the same. As JSON can't have duplicate keys, for elements that may be duplicate in the inbound XML, only the first is selected to be put into the output. Values that are numeric are defined as numbers using the link:/mule-user-guide/v/3.8-m1/dataweave-operators#as[As operator] to avoid having them treated as strings.
...
This example transforms a JSON array of objects, each representing a book, into XML. The link:/mule-user-guide/v/3.8-m1/dataweave-operators#map[*map* operation] carries out the same steps for each element in the input array. Through the use of @, attributes are added into the XML tags.
...
In this example, there are three different input JSON files, these three could all potentially arrive in one single Mule message, occupying variables and properties. One of them contains an array of book objects, another a set of currency exchange rates, and the other one a querry. The transform filters the first input using the conditions passed in the third input, then performs a link:/mule-user-guide/v/3.8-m1/dataweave-operators#map[*map* operation] to deal with each remaining object separately. Within this map, it defines two variables: *it* and *props*. Through the use of @, attributes are added into the XML tags. A second map operation inside the first one calculates the price of each book for each of the currencies provided in the second input. Another map operation displays each element in the "author" array as a separate <author></author> tag.
...
In this example, the input contains sensitive information that should be removed. The transform replicates the inbound structure but uses a simple link:/mule-user-guide/v/3.8-m1/dataweave-operators#remove[*remove* operation] to take away specific key:value pairs. It goes through the whole set of elements in the input using a link:/mule-user-guide/v/3.8-m1/dataweave-operators#map[*map* operation].
...
The following examples makes all the keys lower case. The link:/mule-user-guide/v/3.8-m1/dataweave-language-introduction#the-dataweave-header[DataWeave header] defines a link:/mule-user-guide/v/3.8-m1/dataweave-functions[function] that receives another function as an argument. It then only applies this function to the keys and not the values it processes. The function that is sent as an argument is a very simple one that uses the link:/mule-user-guide/v/3.8-m1/dataweave-operations#lower[lower operator] to set text to lower case.
...
This example shows how, based on a mapping definition that may be stored in a DB table, you can dynamically execute it.

This input describes a mapping definition
...
%input payload application/xml nullValueOn="empty"
%output application/json
